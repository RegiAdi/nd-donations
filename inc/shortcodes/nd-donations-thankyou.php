...
require_once(WP_PLUGIN_DIR . '/nd-donations/vendor/autoload.php');
...
function nd_donations_shortcode_thankyou()
{
	...
	if (isset($_POST['nd_donations_single_cause_form'])) {
		...
		// Xendit
		$is_xendit_test_mode = 0;
		$xendit_api_key = '';
		$invoice = null;

		if (get_option('nd_donations_xendit_test_mode') > 0) {
			$is_xendit_test_mode = 1;
			$xendit_api_key = get_option('nd_donations_xendit_api_key_test');
		} else {
			$is_xendit_test_mode = 0;
			$xendit_api_key = get_option('nd_donations_xendit_api_key_live');
		}

		$params = [
			'external_id' => $nd_donations_tx,
			'payer_email' => $nd_donations_email,
			'description' => 'Pembayaran Donasi Zakahmu - ' . $nd_donations_title_cause,
			'amount' => $nd_donations_value,
		...
		if (!empty($invoice)) {
			// wp_safe_redirect($invoice['invoice_url']);
			// exit;
      ...
      <div class="nd_donations_width_50_percentage nd_donations_float_left nd_donations_box_sizing_border_box nd_donations_padding_15 nd_donations_width_100_percentage_responsive">
        ...
        <h4>' . __('TERIMA KASIH BANYAK ATAS DONASI ANDA', 'nd-donations') . ' :</h4>
		...
		<a class="nd_donations_thankyou_back_to_causes_btn nd_donations_display_inline_block nd_donations_text_align_center nd_donations_box_sizing_border_box nd_donations_width_100_percentage nd_donations_color_white_important nd_donations_bg_green nd_options_first_font nd_donations_padding_10_20 nd_donations_border_radius_30 " 
			href="' . $invoice['invoice_url'] . '" target="_blank">' . __('BAYAR SEKARANG', 'nd-donations') . '</a>   
        <div class="nd_donations_section nd_donations_height_40"></div>
      ...
      <div class="nd_donations_width_50_percentage nd_donations_float_left nd_donations_box_sizing_border_box nd_donations_padding_15 nd_donations_width_100_percentage_responsive">
        ...
        <h4>' . __('SUMMARY OF YOUR DATAS', 'nd-donations') . ' :</h4>
        <div class="nd_donations_section nd_donations_height_30"></div>
